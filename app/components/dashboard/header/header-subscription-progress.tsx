...
export function HeaderSubscriptionProgress() {
    ...
    if (!user || daysLeft === null || user.subscriptionStatus !== "active") {
        return null;
    }
    ...
    return (
        <div className="flex items-center gap-2 sm:gap-3 mr-1">
            ...
            <div
                ...
                title={`${daysLeft} days left in billing cycle`}
            >
                <svg className="size-[34px] -rotate-90" viewBox="0 0 36 36">
                    ...
                    {/* Progress Circle */}
                    <path
                        className={isWarning ? "text-destructive" : "text-[#006c49] dark:text-[#6ffbbe]"}
                ...
                <span className={`absolute text-[10px] font-bold ${isWarning ? "text-destructive" : "text-foreground"}`}>
                    {daysLeft}
                </span>
            </div>
        </div>
