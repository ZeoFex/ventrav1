...
- **Platform writes (superadmin):** Same `X-Ventra-Platform-Key` (no act-as). `GET /api/platform/overview` returns table counts + billing snapshot in one response. `POST /api/platform/users` creates a user in a business. `GET|PATCH|DELETE /api/platform/users/[id]` — GET/DELETE need `?businessId=`; PATCH is either `{ businessId, status }` (suspend, etc.) or a full staff update (like `POST /api/staff`). `GET|PATCH /api/platform/businesses/[id]` read/update business (e.g. plan, `subscriptionStatus`, account `status`, `currentPeriodEnd`, `name`). For catalog/POS data mutations not yet exposed under `/api/platform/*`, use **`X-Act-As-Business-Id`** on the existing tenant APIs or add dedicated platform services later.
